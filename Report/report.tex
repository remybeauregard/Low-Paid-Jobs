...
job-choice decisions. Regression analysis reveals that two stated social influences, family and peer opinion,
are associated with systematic differences in maximum acceptable wages, while
professional development is associated with a higher minimum acceptable wage. These are exploratory patterns intended to inform
the design of a pilot RCT.
\end{abstract}
...
\begin{itemize}
    ...
    sectors where salaries are typically modest, or may exert normative
    pressure against appearing overly ambitious.
...
The raw wage data include implausibly small and large values (e.g.\ a minimum
WTA of GHC~18/month and a maximum WTA of GHC~10,000/month) that likely reflect
measurement or reporting noise rather than genuine preferences. To assess whether these
...
Table~\ref{tab:regsw} reports the results. The winsorisation affected 2 minimum-wage
and 2 maximum-wage observations (those below GHC~100 and above GHC~4,000 for the minimum,
and below GHC~250 for the maximum). All main findings are robust: professional
development remains significant and positive for both the minimum ($p = 0.026$; $p = 0.035$
with controls) and maximum WTA ($p = 0.009$; $p = 0.006$), family opinion remains
negatively associated with the maximum WTA at the 1\% level, and peer opinion remains
positive and significant ($p = 0.005$ without controls; $p = 0.027$ with controls).
The direction, magnitude, and significance of every coefficient are effectively unchanged,
suggesting the original results are not driven by extreme wage reports.

\input{regtable_w_table.tex}
